<template>
    ...
    <div v-else>
        <nuxt-link to="produtosCatalogo/create" v-if="user !== null && user.role != 'Cliente'"
            >Criar novo Produto</nuxt-link
        >
        <h2>Produtos</h2>
        <table>
            <tr>
            ...
            <tr v-for="produto in produtos">
                ...
                <td
                    ...
                    v-if="user.role == 'FabricanteDeProdutos'"
                >
                    <nuxt-link :to="`/produtosCatalogo/${produto.id}`">Detalhes</nuxt-link>
                    <div
                        v-if="
                            user !== null &&
                            user.role == 'FabricanteDeProdutos' &&
                            user.username == produto.fabricanteUsername
                        "
                    >
                        |
...
const {
    ...
    error,
    refresh,
} = await useFetch(`${api}/produtosCatalogo/fabricante/${user.username}`, {
    method: "GET",
